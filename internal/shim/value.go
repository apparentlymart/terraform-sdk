...
func ConfigValueFromHCL2Block(v cty.Value, schema *tfschema.BlockType) map[string]interface{} {
	if v.IsNull() {
		return nil
	}
	if !v.IsKnown() {
	...
	for name, blockS := range schema.NestedBlockTypes {
		...
		case tfschema.NestingSingle:
			ret[name] = ConfigValueFromHCL2Block(bv, &blockS.Content)
		...
		case tfschema.NestingList, tfschema.NestingSet:
			...
			for it := bv.ElementIterator(); it.Next(); {
				...
				elems = append(elems, ConfigValueFromHCL2Block(ev, &blockS.Content))
			}
			ret[name] = elems
		...
		case tfschema.NestingMap:
			...
			for it := bv.ElementIterator(); it.Next(); {
				...
				elems[ek.AsString()] = ConfigValueFromHCL2Block(ev, &blockS.Content)
			}
			ret[name] = elems
		}
	}
